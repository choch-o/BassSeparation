    ...
    def loadInputOutput(self,id):
        ...
        if self.path_transform_in==self.path_transform_out:
            ...
            if self.nsources>1:
                ...
                allmixoutput = allmixoutput[3:]
        # CHANGED
        # allmixinput = np.expand_dims(allmixinput[0], axis=0)
        allmixinput = allmixinput[0:3]
    ...
    def loadFile(self,id,idxbegin=None,idxend=None):
        ...
        if self.path_transform_in is not None and self.path_transform_out is not None:
            ...
            if self.time_context > allmixinput.shape[1]:
                ...
                inputs[0,:allmixinput.shape[1],:allmixinput.shape[-1]] = allmixinput[0]
                ...
                # CHANGED
                for j in range(1,3):
                    inputs[0, :allmixinput.shape[1], j*allmixinput.shape[-1]:(j+1)*allmixinput.shape[-1]] = allmixinput[j]
                # #######
                ...
                while (start + self.time_context) < allmixinput.shape[1]:
                    if i>=idxbegin and i<idxend:
                        ...
                        inputs[i-idxbegin, :, :allminput.shape[-1]] = allminput[0]
                        ...
                        # CHANGED
                        for j in range(1,3):
                            inputs[i-idxbegin,:, j*allminput.shape[-1]:(j+1)*allminput.shape[-1]] = allminput[j,:,:]
                        ########
                        for j in range(1,self.nsources):
            ...
            for i in range(len(self.file_list)):
                if os.path.isfile(os.path.join(self.path_transform_in[self.dirid[i]],self.file_list[i])):
                    ...
                    if self.path_transform_in==self.path_transform_out:
                        ...
                        return 3 * allmix.shape[-1], self.nsources * allmix.shape[-1]
                    else:
                        ...
                        return 3 * allmix.shape[-1], self.nsources * allmixoutput.shape[-1]
