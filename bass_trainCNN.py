...
def build_ca(input_var=None, batch_size=32,time_context=30,feat_size=513):
    ...
    feat_size = feat_size/3
    # CHANGED
    input_shape=(batch_size,1,time_context,feat_size)
    # input_shape=(batch_size,3,time_context,feat_size)

    #input layer
    l_in_1 = lasagne.layers.InputLayer(shape=input_shape, input_var=input_var[:,0:1,:,:])
    ...
    l_in_lpf100 = lasagne.layers.InputLayer(shape=input_shape, input_var=input_var[:,1:2,:,:])
    l_in_lpf262 = lasagne.layers.InputLayer(shape=input_shape, input_var=input_var[:,2:3,:,:])

    #vertical convolution layer
    l_conv1_lpf100 = lasagne.layers.Conv2DLayer(l_in_lpf100, num_filters=50, filter_size=(1,feat_size),stride=(1,1), pad='valid', nonlinearity=None)
    l_conv1b_lpf100 = lasagne.layers.BiasLayer(l_conv1_lpf100)

    #horizontal convolution layer
    l_conv2_lpf100 = lasagne.layers.Conv2DLayer(l_conv1b_lpf100, num_filters=50, filter_size=(int(time_context/2),1),stride=(1,1), pad='valid', nonlinearity=None)
    l_conv2b_lpf100 = lasagne.layers.BiasLayer(l_conv2_lpf100)


    #vertical convolution layer
    l_conv1_lpf262 = lasagne.layers.Conv2DLayer(l_in_lpf262, num_filters=50, filter_size=(1,feat_size),stride=(1,1), pad='valid', nonlinearity=None)
    l_conv1b_lpf262 = lasagne.layers.BiasLayer(l_conv1_lpf262)

    #horizontal convolution layer
    l_conv2_lpf262 = lasagne.layers.Conv2DLayer(l_conv1b_lpf262, num_filters=50, filter_size=(int(time_context/2),1),stride=(1,1), pad='valid', nonlinearity=None)
    l_conv2b_lpf262 = lasagne.layers.BiasLayer(l_conv2_lpf262)

    l_conv2b_concat = lasagne.layers.ConcatLayer([l_conv2b, l_conv2b_lpf100, l_conv2b_lpf262])
    #bottlneck layer
    l_fc=lasagne.layers.DenseLayer(l_conv2b_concat,128)
...
def train_auto(train,fun,transform,testdir,outdir,num_epochs=30,model="1.pkl",scale_factor=0.3,load=False,skip_train=False,skip_sep=False):
    ...
    rand_num = np.random.uniform(size=(train.batch_size,1,train.time_context,train.input_size/3))
    ...
    bass=mask2*input_var2[:,0:1,:,:]
    drums=mask3*input_var2[:,0:1,:,:]
    others=mask4*input_var2[:,0:1,:,:]
    ...
    # train_loss_recon_neg = beta*lasagne.objectives.squared_error(vocals,target_var2[:,3:4,:,:])
    train_loss_recon_neg = beta*lasagne.objectives.squared_error(vocals,target_var2[:,1:2,:,:])
    ...
    train_loss_recon_neg_bass = beta_bass*lasagne.objectives.squared_error(bass,target_var2[:,2:3,:,:])
    ...
    train_loss_recon_neg += beta*lasagne.objectives.squared_error(drums,target_var2[:,1:2,:,:])
    ...
    negative_error_bass=train_loss_recon_neg_bass.sum()
    ...
    loss=abs(vocals_error+drums_error+bass_error-negative_error-alpha_component-negative_error_bass)
    ...
    train_fn1 = theano.function([input_var2,target_var2], [vocals_error,bass_error,drums_error,negative_error,alpha_component,negative_error_bass], allow_input_downcast=True)
    ...
    if not skip_train:
        ...
        for epoch in range(num_epochs):
            ...
            for batch in range(train.iteration_size):
                ...
                inputs=np.ndarray(shape=(input_.shape[0],3,input_.shape[1],jump))
                # targets=np.ndarray(shape=(inputs.shape[0],4,inputs.shape[2],inputs.shape[3]))
                targets=np.ndarray(shape=(inputs.shape[0],4,inputs.shape[2],jump))

                inputs[:,0,:,:]=input_[:,:,:jump]
                inputs[:,1,:,:]=input_[:,:,jump:jump*2]
                inputs[:,2,:,:]=input_[:,:,jump*2:jump*3]
                input_ = None
                ...
                [vocals_erre,bass_erre,drums_erre,negative_erre,alpha,betae_bass]=train_fn1(inputs,targets)
                vocals_err +=vocals_erre
                bass_err +=bass_erre
                drums_err +=drums_erre
                negative_err +=negative_erre
                # beta_voc+=betae_voc
                beta_bass+=betae_bass
            ...
            print("  Beta component for bass:\t\t{:.6f}".format(beta_bass/train_batches))
    ...
    if not skip_sep:
        ...
        dev_directory = os.listdir(os.path.join(testdir,"train"))
        ...
        for f in sorted(dirlist):
            if not f.startswith('.'):
                if f in dev_directory:
                    song=os.path.join(testdir,"train",f,"mixture.wav")
                    song_lpf100=os.path.join(testdir,"train",f,"lpf_100.wav")
                    song_lpf262=os.path.join(testdir,"train",f,"lpf_262.wav")
                else:
                    song=os.path.join(testdir,"test",f,"mixture.wav")
                    song_lpf100=os.path.join(testdir,"test",f,"lpf_100.wav")
                    song_lpf262=os.path.join(testdir,"test",f,"lpf_262.wav")
                ...
                # LPF100 mixture
                audio_lpf100, sr, br = util.readAudioScipy(song_lpf100)
                mag_lpf100,ph_lpf100=transform.compute_file(audio_lpf100,phase=True)

                assert sr == 44100,"Sample rate needs to be 44100"
                mag_lpf100=scale_factor*mag_lpf100.astype(np.float32)

                lpf100_batches,lpf100_nchunks = util.generate_overlapadd(mag_lpf100,input_size=mag_lpf100.shape[-1],time_context=train.time_context,overlap=train.overlap,batch_size=train.batch_size,sampleRate=sr)


                # LPF100 mixture
                audio_lpf262, sr, br = util.readAudioScipy(song_lpf262)
                mag_lpf262,ph_lpf262=transform.compute_file(audio_lpf262,phase=True)

                assert sr == 44100,"Sample rate needs to be 44100"
                mag_lpf262=scale_factor*mag_lpf262.astype(np.float32)

                lpf262_batches,lpf262_nchunks = util.generate_overlapadd(mag_lpf262,input_size=mag_lpf262.shape[-1],time_context=train.time_context,overlap=train.overlap,batch_size=train.batch_size,sampleRate=sr)
                ...
                # for batch in batches:
                    batch = batches[i]
                    lpf100_batch = lpf100_batches[i]
                    lpf262_batch = lpf262_batches[i]
                    batch_no+=1
                    start_time=time.time()
                    lpf_batch=np.ndarray(shape=(batch.shape[0],3,batch.shape[2],batch.shape[3]))
                    lpf_batch[:,0:1,:,:] = batch
                    lpf_batch[:,1:2,:,:] = lpf100_batch
                    lpf_batch[:,2:3,:,:] = lpf262_batch

                    output.append(predict_function2(lpf_batch))
                ...
                #write audio files
                if f in dev_directory:
                    dirout=os.path.join(outdir,"train",f)
                else:
...
if __name__ == "__main__":
    ...
    train_errs=train_auto(train=ld1,fun=build_ca,transform=tt,outdir=os.path.join(db,'output',model),testdir=os.path.join(db,''),model=os.path.join(db,'models',"model_"+model+".pkl"),num_epochs=nepochs,scale_factor=scale_factor)
