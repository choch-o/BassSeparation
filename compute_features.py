    ...
    for f in sorted(dirlist):
        if not f.startswith('.'):
            ...
            if vocals.shape[1]>1:
                ...
                vocals = vocals[:,0]
            bass, sampleRate, bitrate = util.readAudioScipy(os.path.join(db,"train",f,"bass.wav"))
            if bass.shape[1]>1:
                bass[:,0] = (bass[:,0] + bass[:,1]) / 2
                bass = bass[:,0]
            drums, sampleRate, bitrate = util.readAudioScipy(os.path.join(db,"train",f,"drums.wav"))
            if drums.shape[1]>1:
                drums[:,0] = (drums[:,0] + drums[:,1]) / 2
                drums = drums[:,0]
            others, sampleRate, bitrate = util.readAudioScipy(os.path.join(db,"train",f,"other.wav"))
            if others.shape[1]>1:
            ...
            for i in range(number_of_blocks):
                audio = np.zeros((sampleRate*30,7))
                audio[:,0]=mix_raw[i*30*sampleRate:(i+1)*30*sampleRate]
                audio[:,1]=lpf_100[i*30*sampleRate:(i+1)*30*sampleRate]
                audio[:,2]=lpf_262[i*30*sampleRate:(i+1)*30*sampleRate]
                audio[:,3]=vocals[i*sampleRate*30:(i+1)*30*sampleRate]
                audio[:,4]=bass[i*sampleRate*30:(i+1)*sampleRate*30]
                audio[:,5]=drums[i*sampleRate*30:(i+1)*sampleRate*30]
                audio[:,6]=others[i*sampleRate*30:(i+1)*sampleRate*30]
            ...
            audio = np.zeros((len(rest),7))
            audio[:,0]=rest
            audio[:,1]=lpf_100[(i+1)*30*sampleRate:]
            audio[:,2]=lpf_262[(i+1)*30*sampleRate:]
            audio[:,3]=vocals[(i+1)*30*sampleRate:]
            audio[:,4]=bass[sampleRate*30*(i+1):]
            audio[:,5]=drums[sampleRate*30*(i+1):]
            audio[:,6]=others[sampleRate*30*(i+1):]
